...
document.addEventListener('astro:before-preparation', event => {
  ...
  event.signal.addEventListener('abort', () => {
    if (generation === current) { cancelAnimations(); prepared = false; previous.clear(); }
  }, { once: true });
  event.loader = async () => {
    await load();
...
document.addEventListener('astro:after-swap', () => {
  running.forEach(animation => animation.cancel());
  running.clear();
  if (!prepared || motion.matches) { cancelAnimations(); return; }
  ...
  entering.forEach(element => {
    ...
    animation.finished.then(() => { animation.cancel(); running.delete(animation); }, () => {});
  });
...
window.addEventListener('pagehide', () => { cancelAnimations(); prepared = false; previous.clear(); });
