...
function initSearch(root: HTMLElement, signal: AbortSignal) {
    ...
    function search(query: string) {
        ...
        const matching = new Set(entries.filter(entry => tokens.every(token => (
            token.startsWith('#') ? entry.tags.some(tag => tag.includes(token.slice(1))) : entry.text.includes(token)
        ))).map(entry => entry.id));
        cards.forEach(card => {
            card.hidden = !matching.has(card.dataset.postId!);
        });
