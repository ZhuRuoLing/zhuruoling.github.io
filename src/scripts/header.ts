...
function initBio(root: HTMLElement, motion: MediaQueryList) {
  ...
  update();
  return () => { clearInterval(cycle); settle(); motion.removeEventListener('change', update); };
}
...
function stopHeader() { cleanup?.(); cleanup = undefined; }
function startHeader() {
  ...
  cleanup = () => { controller.abort(); cleanBio(); cleanOverlay(); };
}
...
window.addEventListener('pageshow', event => { if (event.persisted) startHeader(); });
